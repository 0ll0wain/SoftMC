...
InstructionSequence::~InstructionSequence() { delete[] instrs; }
...
//! Generates an instruction to \b write a single byte pattern to the given
//! bank/column address.
/*!
  \param \e col is the column number.
  \param \e bank is the bank number.
  \param \e pattern is the byte that will be written to the entire column.
  \param \e ap is the auto-precharge option with default value
 AUTO_PRECHARGE.NO_AP.
...
*/
Instruction genWR(uint bank, uint col, uint8_t pattern, AUTO_PRECHARGE ap,
                  BURST_LENGTH bl) {
  Instruction instr = (uint)INSTR_TYPE::DDR;
...
Instruction genRD(uint bank, uint col, AUTO_PRECHARGE ap, BURST_LENGTH bl) {
  Instruction instr =
      0x25; // to set CKE(1) CS(0)[assumed it should be Low] RAS(1) CAS(0) WE(1)
...
Instruction genWAIT(uint cycles) { // min 1, max 1023
  assert(cycles >= 1);
  assert(cycles <= 1023 && "Could not wait for more than 1023 cycles since the "
                           "current hardware implementation has a 10 bit "
...
/*!
  \param \e dir is the new bus mode to be set. Can be BUSDIR.READ to switch to
read
...
//! Generates an instruction to indicate the <b> end of the instruction sequence
//! </b>.
/*!
...
//! Generates an instruction for configuring the CLK speed.
/*!
  \param \e clkfbout_mult is the value to be set for CLKFBOUT_MULT in the PLL.
  Should be >=5.
  \param \e divclk_divide is the value to be set for DIVCLK_DIVIDE in the PLL.
  \param \e clkout_divide is the value to be set for CLKOUT_DIVIDE in the PLL.
  \return The generated command for auto-refresh configuration
  CLK Speed is calculated as follows:
  DDR_clk = (base_clk * clkfbout_mult) / (divclk_divide * clkout_divide)
  fabric_clk = DDR_clk / 2
  Example 1:
  base_clk = 400 MHz, clkfbout_mult = 6 , divclk_divide = 1, clkout_divide = 3
  DDR_clk = (400 MHz * 6) / (1 * 3) = 2400 MHz / 3 = 800 MHz
  fabric_clk = 800 MHz / 2 = 400 MHz
  Example 2:
  base_clk = 400 MHz, clkfbout_mult = 5 , divclk_divide = 2,clkout_divide = 1
  DDR_clk = (400 MHz * 5) / (2 * 1) = 20000 MHz / 2 = 10000 MHz
  fabric_clk = 10000 MHz / 2 = 5000 MHz
*/
Instruction genCLKSPEED_CONFIG(uint clkfbout_mult, uint divclk_divide,
                               uint clkout_divide) {

  assert(clkfbout_mult > 5);

  Instruction instr = 0x50;
  instr <<= 8;
  instr |= clkout_divide;
  instr <<= 8;
  instr |= divclk_divide;
...
//! Generates an instruction to set the CLK speed back to default.
/*!
  \return The generated command for auto-refresh configuration
*/
Instruction genCLKSPEED_DEFAULT() {

  Instruction instr = 0x58000000;
