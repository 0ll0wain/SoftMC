...
#define TIME_VAL_TO_MS(num)                                                    \
  (((double)_timers[num].tv_sec * 1000.0) +                                    \
   ((double)_timers[num].tv_usec / 1000.0))
...
enum class INSTR_TYPE { END_OF_INSTRS = 0, SET_BUS_DIR = 1, WAIT = 4, DDR = 8 };
// END - DO NOT EDIT

enum class BUSDIR { READ = 0, WRITE = 2 };

enum class AUTO_PRECHARGE { NO_AP = 0, AP = 1 };

enum class PRE_TYPE { SINGLE = 0, ALL = 1 };

enum class BURST_LENGTH { CHOP = 0, FIXED = 1 };

enum class REGISTER { TREFI = 2, TRFC = 3 };
...
Instruction genWR(uint bank, uint col, uint8_t pattern,
                  AUTO_PRECHARGE ap = AUTO_PRECHARGE::NO_AP,
                  BURST_LENGTH bl = BURST_LENGTH::FIXED);
Instruction genRD(uint bank, uint col,
                  AUTO_PRECHARGE ap = AUTO_PRECHARGE::NO_AP,
                  BURST_LENGTH bl = BURST_LENGTH::FIXED);
