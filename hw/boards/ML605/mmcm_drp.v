...
module mmcm_drp
   #(
      ...
      input             SADDR,			//0 = Default State1, 1 = Programmable
      input             SEN,				//Reprogramming Enable
      input             SCLK,				//System CLK
      input             RST,				//Reset
      output reg        SRDY,
		
		input			PAR_WR_EN,	//Parameter Write Enable
		input 		[7:0]	CLKFBOUT_MULT,
		input			[7:0]	DIVCLK_DIVIDE,
		input			[7:0]	CLKOUT_DIVIDE, //Divide für den DRAM, der divide für das modul wird verdoppelt -> halbe frequenz
   ...
   assign DCLK = SCLK;
				...
				reg [37:0] S2_CLKFBOUT   ;
					
				reg [9:0] S2_DIGITAL_FILT  ;
	...
	always @(posedge SCLK) begin
      if(PAR_WR_EN) begin
         S2_CLKFBOUT_MULT <= #TCQ CLKFBOUT_MULT;
			...
			S2_CLKOUT0_DIVIDE <= #TCQ DIVCLK_DIVIDE * 2;
			S2_CLKOUT1_DIVIDE <= #TCQ DIVCLK_DIVIDE;
			S2_CLKOUT2_DIVIDE <= #TCQ DIVCLK_DIVIDE;
      end
   end
      ...
      case (current_state)
         ...
         WAIT_SEN: begin
			...
			BIT_CALC: begin
				...
				S2_LOCK     		= mmcm_lock_lookup(S2_CLKFBOUT_MULT);
				
				S2_DIVCLK       	= mmcm_count_calc(S2_DIVCLK_DIVIDE, 0, 50000); 
				
				S2_CLKOUT0      	= mmcm_count_calc(S2_CLKOUT0_DIVIDE, S2_CLKOUT0_PHASE, S2_CLKOUT0_DUTY);
